...
import  throttle from 'lodash/throttle.js'
  export default {
    ...
    methods:{
      ...
      //修改产品个数,通过不同传参实现加减改,节流
      handler:throttle(async function(type,disNum,cart){
        //type 区分3个元素
        //disNum 传参，传加减+-1是变化量  input最终量  cart传参确认点击的是哪个商品【通过ID】
        //向服务器发请求修改数量,addOrUpdateShopCart (skuId,skuNum) 第一个是id 第二个接口是修改的差值
        switch(type){
          //加号 
          case "add": disNum = 1
          //带给服务器变化的量
    ...
    computed:{
      ...mapGetters(['cartList']),
        ...
        //判断底部复选框是否全部勾选 every方法 全部元素都是1返回true
        isAllCheck(){
